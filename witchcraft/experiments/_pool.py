"""witchcraft.experiments._pool — guarded multiprocessing for the engine-bound work (self-play gen + gauntlet).
...
def _selfplay_worker(args):
    ...
    import torch
    from witchcraft.magezero import MageZeroNet, generate_selfplay
...
def _gauntlet_worker(args):
    ...
    import torch
    from witchcraft.magezero import MageZeroNet, MageZeroPlayer
...
def parallel_gauntlet(net_path: str, *, embed: int, hidden: int, sims: int, rungs: list[str], games: int,
    ...
    from witchcraft.ladder import score_stats
