"""witchcraft.cards — the whole card corpus as one importable, iterable thing.
...
The corpus is lazy and cached: importing `witchcraft` stays cheap, and the ~14 MB JSON is read only
...
class CardCorpus:
    ...
    testable (`"Black Lotus" in cards`). The single instance exported as `witchcraft.cards`."""
    ...
    def __repr__(self) -> str:                                  # never forces a load
        ...
        return f"<witchcraft.cards: {len(loaded[1])} cards>" if loaded else \
               "<witchcraft.cards: MTGJSON card corpus (lazy — iterate to load)>"
