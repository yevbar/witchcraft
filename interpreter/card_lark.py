...
from interpreter.card_effects import Effect, _target, _amount, _is_compound_object, _kw_ok, _kw_list, _TGT, _LIB_OWNER, _mana_production, _SIDED
from interpreter import card_effects as _ce
...
BECOMESDESIG.6: /\bbecomes? (?:the monarch|foretold|plotted|blocked|snow|saddled|unprepared|prepared|monarch)\b/   // '<subj> becomes foretold/plotted' — §701 status designation bigram (outranks BCM_COP); + §509 'becomes blocked' (forced-block state change) + §205 'becomes snow' (supertype set) + §702 'becomes saddled' (the saddle keyword's status, OTJ 2024 — saddle is a grounded keyword ability) + §700 'becomes [un]prepared' (Bloomburrow/FF prepare designation; 'unprepared' before 'prepared' so it wins the alternation) + §720 'becomes the monarch' (a targeted monarch designation, distinct from the 'you become the monarch' litclause) — all the same becomes(-, subj, <state>) shape
...
from interpreter.card_effects import _TGT as _BCM_TGT_SRC
from interpreter.card_effects import _that_amt as _that_amt   # 'twice/half that much [plus N]' amount slug — reused, not re-derived
...
from interpreter.card_effects import _OBJ_BAD as _OBJ_BAD
...
from interpreter.card_effects import _COPULA_RUNON as _BT_RUNON
