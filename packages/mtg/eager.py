"""mtg.eager — a player that picks its plan from the deck's WIN CONDITIONS.

Where `HeuristicPlayer` hand-codes one general policy and `AggroPlayer` always races damage, `EagerPlayer` first
asks the engine which win axes its deck can even pursue (`game.win_conditions`, from `mtg.wincon`) and
