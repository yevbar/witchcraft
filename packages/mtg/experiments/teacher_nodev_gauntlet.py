"""Decisive Definition-of-Done gate: the REAL candidate (c8801e7) = HeuristicPlayer fallback + sound forced
KILL, develop=False, on the full gauntlet at a real N with CIs, plus the last_arm distribution (how often the
kill actually fires vs the heuristic fallback). develop=False -> no find_progress every move -> fast, so N is
high. Parallel one job per rung + one last_arm job."""
import time, multiprocessing as mp, collections

WIN_BUDGET, LIFE_GATE, MAXM, N = 800, 14, 2500, 150
...
def teacher():
    ...
    return SolverSeekingPlayer(HeuristicPlayer(), axis="life_zero", win_turns=1, win_budget=WIN_BUDGET,
                               life_gate=LIFE_GATE, develop=False, forced=True)
