# Packaging `mtg` like python-chess — deferred work

The `mtg/` package gives an importable, python-chess-shaped API (`mtg.Game`). What it does
...
## What blocks a clean `pip install python-mtg`
...
2. **Engine modules live at the repo root, not in the package.** `driver`, `env`, `game`,
   ...
   `mtg._engine.driver`) with the bare imports rewritten, or kept as a separately-published
   `mtg-engine` dependency.
...
2. Move the engine modules into the package (or split a `mtg-engine` dist) — mechanical, larger diff.
