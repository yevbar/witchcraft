...
def _m2_root_cap_ordering() -> None:
    ...
    class FG:                                                  # minimal stand-in Game (only .state/.turn read)
        def __init__(self, state, turn):
            self.state, self.turn = state, turn
    fg = FG({}, "alice")
    ...
    chosen = ReBeLPlayer(action_cap=4, policy_fn=pf, cap_floor=1, seed=0)._root_actions(fg, moves)
    ...
    # order_cap=False AND no policy_fn -> the legacy alphabetical prefix moves[:cap] (the default order_cap=True
    # value-orders, which needs a real state; the policy prior, when present, takes precedence over both).
    leg = ReBeLPlayer(action_cap=4, order_cap=False, seed=0)._root_actions(fg, moves)
    check("no ordering -> legacy alphabetical prefix moves[:cap]", [m.tag for m in leg] == [0, 1, 2, 3])
