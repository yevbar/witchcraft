...
def matchup_winrate(net_vf, my_deck, opp_deck, games: int, seed: int = 300, move_cap: int = 400):
    """ValuePlayer(net) piloting my_deck vs RandomPlayer piloting opp_deck — pilot+deck swap seats together.
    Games are capped at `move_cap` moves to tame grindy deck-out games (ValuePlayer is slow per decision);
    a game that hits the cap is decided on LIFE TOTAL (a fair proxy for who's ahead). Returns (winrate, n_capped)."""
    wins = capped = 0
    for i in range(games):
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            g = play(players, decks, variant="two-player", seed=seed + i, max_moves=move_cap)
        w = g.winner()
...
def main():
    ...
    ap.add_argument("--eval", type=int, default=30, help="games per matchup")
    ...
    print(f"\nValuePlayer({a.deck}) vs Random across MATCHUPS ({a.eval} games each, seat-swapped, "
          f"cap {a.cap} moves):", flush=True)
    for opp in bundled_decks():
        wr, capped = matchup_winrate(vf, my, load_deck(opp), a.eval, move_cap=a.cap)
